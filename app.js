...
//mongodb://localhost:27017/relianceDB
...
app.post("/shareCredit", function(req, res){
  ...
  User.findOne({username: req.body.username}, function(err, foundUser){
    ...
    if(req.body.shares == "100"){
      newValue = foundUser.earnings.totalShare + 100
    }
    if(req.body.shares == "200"){
      newValue = foundUser.earnings.totalShare + 200
    }
    if(req.body.shares == "500"){
      newValue = foundUser.earnings.totalShare + 500
    }
    if(req.body.shares == "1000"){
      newValue = foundUser.earnings.totalShare +1000
    }
    if(req.body.shares == "0"){
      newValue = foundUser.earnings.totalShare
    }
    User.updateOne({username: req.body.username}, {$set:{earnings:{currentShare: req.body.shares, totalShare: newValue, availableBalance: foundUser.earnings.availableBalance }}}, function(error){
      if(error){
        console.log(error);
      }
    });
...
app.post("/amountCredit", function(req, res){
  User.findOne({username: req.body.username}, function(err, foundUser){
    let newValue = 0;
    if (req.body.amount !== "908908"){
      newValue = foundUser.earnings.availableBalance
    }
    if(req.body.amount == "13000"){
      newValue = foundUser.earnings.availableBalance + 13000
    }
    if(req.body.amount == "26000"){
      newValue = foundUser.earnings.availableBalance + 26000
    }
    if(req.body.amount == "65000"){
      newValue = foundUser.earnings.availableBalance + 65000
    }
    if(req.body.amount == "130000"){
      newValue = foundUser.earnings.availableBalance +130000
    }
    if(req.body.amount == "0"){
      newValue = foundUser.earnings.availableBalance
    }

    User.updateOne({username: req.body.username}, {$set:{earnings:{currentShare: foundUser.earnings.currentShare, totalShare: foundUser.earnings.totalShare, availableBalance: newValue }}}, function(error){
      if(error){
        console.log(error);
      }
    });
